...
contract('RRC20', (accounts) => {
  ...
  it('Transfers: Should fail when trying to transfer 10001 to accounts[1] with accounts[0] having 10000', async () => {
    await truffleAssert.reverts((HST.transfer.call(accounts[1], 10001, { from: accounts[0] })),"VM Exception while processing transaction: transaction reverted");
  });
  ...
  it('Approvals: msg.sender approves accounts[1] of 100 & withdraws 50 & 60 (2nd tx should fail)', async () => {
    ...
    await truffleAssert.reverts(HST.transferFrom.call(accounts[0], accounts[2], 60, { from: accounts[1] }),"VM Exception while processing transaction: transaction reverted");
  });
  ...
  it('Approvals: Allow accounts[1] 100 to withdraw from accounts[0]. Withdraw 60 and then approve 0 & attempt transfer.', async () => {
    ...
    await truffleAssert.reverts(HST.transferFrom.call(accounts[0], accounts[2], 10, { from: accounts[1] }),"VM Exception while processing transaction: transaction reverted");
  });
